...
import fr.nivcoo.edenquests.api.tracking.TrackingSession;
...
final class EdenQuestsChallengeTrackingService implements ChallengeTrackingService {
    ...
    public ChallengeTrackingSession activate(String definitionId, UUID runId, Instant startsAt, Instant endsAt,
        ...
        TrackingSession session = catalog.activate(
                definitionId,
                runId,
                edenDigest,
                startsAt,
                endsAt,
                progress -> {
                    final BigDecimal signedDelta;
                    try {
                        signedDelta = progress.signedDelta();
                        ChallengeAmount.parseDelta(
                                ChallengeAmount.canonical(signedDelta));
